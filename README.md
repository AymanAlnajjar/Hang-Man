...
3+ letters (swipe to trace; longer words score exponentially more). The match is
**3 synchronized, timer-driven rounds** — all grids are generated up front so you
both see the same letters each round, and rounds auto-advance on a shared clock
(no buttons). The higher score each round earns a **⭐**; most stars wins the
match. At each reveal (and the end) you see **both players' words**. Each grid is
seeded with real words along adjacent paths, so there's always something to find.

**Dictionary:** the grid game validates against ~30,000 common Arabic words,
each **3–9 letters** (longer words can't be traced on a 5×5 board, so they're
...
| perl -CSD -Mutf8 -ne 'BEGIN{%ok=map{$_=>1}split//,"ابتثجحخدذرزسشصضطظعغفقكلمنهويةء";} last if $n>=30000; ($w)=split/\s+/; $w=~s/[\x{064B}-\x{0652}\x{0670}\x{0640}]//g; $w=~tr/\x{0623}\x{0625}\x{0622}\x{0671}/\x{0627}/; $w=~tr/\x{0649}/\x{064A}/; $w=~tr/\x{0624}/\x{0648}/; $w=~tr/\x{0626}/\x{064A}/; $l=length $w; next if $l<3||$l>9; $g=1; for $c (split//,$w){ if(!$ok{$c}){$g=0;last;} } next unless $g; next if $seen{$w}++; print "$w\n"; $n++;' \
> public/ar-words.txt
```

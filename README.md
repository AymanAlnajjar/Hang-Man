...
[`lib/words.ts`](./lib/words.ts). The word-grid **dictionary** (used both to
validate words and to plant them) lives in
[`lib/dictionary.ts`](./lib/dictionary.ts) — add words there to make the grid
game richer. Grid generation and scoring live in [`lib/grid.ts`](./lib/grid.ts).
